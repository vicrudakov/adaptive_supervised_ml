...
### Functions
{
  ...
  read_results_baselines <- function(file_path) {
    results_df <- read.csv(file_path) %>%
  ...
  read_time_baselines <- function(folder_path) {
    read_time_file <- function(file_path) {
      time_df <- read.csv(file_path) %>%
        separate(experiment, into = c(NA, "Dataset", "Total size", "PEFT module", "AL method"), 
                 sep = "_",  extra = "merge") %>%
  ...
  create_plot_results <- function(results_df_training, results_df_baselines, total_size, file_path) {
    ...
    plt <- results_df %>%
      ...
      scale_x_continuous(breaks = 0:9, labels = total_size / 10 * 1:10) +
  ...
  create_plot_speedup <- function(results_df_training, results_df_baselines, 
    ...
    # 9. Bind everything together, rename methods, apply ordering, and plot
    plt <- bind_rows(combined_train, combined_baseline) %>%
      ...
      ylab(paste("F1 score", ifelse(type == "relative", "(relative)", ifelse(type == "absolute", "(absolute)", NA)))) +
      xlab("Speedup") +
  ...
  create_results_table_short <- function(results_df_training, results_df_baselines, total_size, file_path) {
    ...
    for (i in seq_along(peft_modules)) {
      ...
      latex_subtable <- subtable_df %>%
        ...
        add_header_above(
          setNames(c(2, n_f1_cols), c("Methods", "F1 score")), 
          bold = TRUE
        ) %>%
        ...
        gsub("\\\\multicolumn\\{2\\}\\{[^\\}]+\\}\\{\\\\textbf\\{Methods\\}\\}", 
  ...
  create_results_table_full <- function(results_df_training, results_df_baselines, total_size, peft_module, file_path) {
    ...
    table_tex_top <- table_tex_top_df %>% 
      ...
      row_spec(0, bold = TRUE) %>%
      add_header_above(
        setNames(c(2, n_f1_top), c("Methods", "F1 score")), 
        bold = TRUE
      ) %>%
      collapse_rows(columns = 1, valign = "middle", latex_hline = "custom",
                    custom_latex_hline = 1) %>%
    ...
    table_tex_bottom <- table_tex_bottom_df %>% 
      ...
      row_spec(0, bold = TRUE) %>%
      add_header_above(
        setNames(c(2, n_f1_bot), c("Methods", "F1 score")), 
        bold = TRUE
      ) %>%
      collapse_rows(columns = 1, valign = "middle", latex_hline = "custom",
                    custom_latex_hline = 1) %>%
  ...
  create_time_table_short <- function(time_df_training, time_df_baselines, total_size, file_path) {
    ...
    for (i in seq_along(peft_modules)) {
      ...
      latex_subtable <- subtable_df %>%
        ...
        row_spec(0, bold = TRUE) %>%
        add_header_above(
          setNames(c(2, n_time_cols), c("Methods", "Time (min)")), 
          bold = TRUE
        ) %>%
        collapse_rows(columns = 1, valign = "middle", latex_hline = "custom",
                      custom_latex_hline = 1) %>%
  ...
  create_time_table_full <- function(time_df_training, time_df_baselines, total_size, peft_module, file_path) {
    ...
    table_tex_top <- table_tex_top_df %>% 
      ...
      row_spec(0, bold = TRUE) %>%
      add_header_above(
        setNames(c(2, n_time_top), c("Methods", "Time (min)")), 
        bold = TRUE
      ) %>%
      collapse_rows(columns = 1, valign = "middle", latex_hline = "custom",
                    custom_latex_hline = 1) %>%
    ...
    table_tex_bottom <- table_tex_bottom_df %>% 
      ...
      row_spec(0, bold = TRUE) %>%
      add_header_above(
        setNames(c(2, n_time_bot), c("Methods", "Time (min)")), 
        bold = TRUE
      ) %>%
      collapse_rows(columns = 1, valign = "middle", latex_hline = "custom",
                    custom_latex_hline = 1) %>%
  ...
  create_speedup_table_short <- function(time_df_training, time_df_baselines, total_size, file_path) {
    dataset <- time_df_training$Dataset[1]
    ...
    for (i in seq_along(peft_modules)) {
      ...
      latex_subtable <- subtable_df %>%
        ...
        row_spec(0, bold = TRUE) %>%
        add_header_above(
          setNames(c(2, n_speedup_cols), c("Methods", "Speedup")), 
          bold = TRUE
        ) %>%
        collapse_rows(columns = 1, valign = "middle", latex_hline = "custom",
                      custom_latex_hline = 1) %>%
...
results_df_baselines_agnews <- read_results_baselines("results_baselines/agnews/results_f1.csv")
results_df_baselines_sensation <- read_results_baselines("results_baselines/sensation/results_f1.csv")
results_df_baselines_trec <- read_results_baselines("results_baselines/trec/results_f1.csv")
...
time_df_baselines_agnews <- read_time_baselines("time_baselines/agnews/")
time_df_baselines_sensation <- read_time_baselines("time_baselines/sensation/")
time_df_baselines_trec <- read_time_baselines("time_baselines/trec/")
...
create_results_table_short(results_df_training_agnews, results_df_baselines_agnews, 
                           10, "tables_training/results_short/results_short_agnews_10.tex")
create_results_table_short(results_df_training_sensation, results_df_baselines_sensation,
                           10, "tables_training/results_short/results_short_sensation_10.tex")
create_results_table_short(results_df_training_trec, results_df_baselines_trec,
                           10, "tables_training/results_short/results_short_trec_10.tex")

create_results_table_full(results_df_training_agnews, results_df_baselines_agnews,
                          10, "adapter", "tables_training/results_full/results_full_agnews_10_adapter.tex")
create_results_table_full(results_df_training_agnews, results_df_baselines_agnews,
                          10, "lora", "tables_training/results_full/results_full_agnews_10_lora.tex")
create_results_table_full(results_df_training_agnews, results_df_baselines_agnews,
                          10, "prefix", "tables_training/results_full/results_full_agnews_10_prefix.tex")
create_results_table_full(results_df_training_sensation, results_df_baselines_sensation,
                          10, "adapter", "tables_training/results_full/results_full_sensation_10_adapter.tex")
create_results_table_full(results_df_training_sensation, results_df_baselines_sensation,
                          10, "lora", "tables_training/results_full/results_full_sensation_10_lora.tex")
create_results_table_full(results_df_training_sensation, results_df_baselines_sensation,
                          10, "prefix", "tables_training/results_full/results_full_sensation_10_prefix.tex")
create_results_table_full(results_df_training_trec, results_df_baselines_trec, 
                          10, "adapter", "tables_training/results_full/results_full_trec_10_adapter.tex")
create_results_table_full(results_df_training_trec, results_df_baselines_trec,
                          10, "lora", "tables_training/results_full/results_full_trec_10_lora.tex")
create_results_table_full(results_df_training_trec, results_df_baselines_trec,
                          10, "prefix", "tables_training/results_full/results_full_trec_10_prefix.tex")

create_time_table_short(time_df_training_agnews, time_df_baselines_agnews, 
                        10, "tables_training/time_short/time_short_agnews_10.tex")
create_time_table_short(time_df_training_sensation, time_df_baselines_sensation,
                        10, "tables_training/time_short/time_short_sensation_10.tex")
create_time_table_short(time_df_training_trec, time_df_baselines_trec,
                        10, "tables_training/time_short/time_short_trec_10.tex")

create_time_table_full(time_df_training_agnews, time_df_baselines_agnews,
                       10, "adapter", "tables_training/time_full/time_full_agnews_10_adapter.tex")
create_time_table_full(time_df_training_agnews, time_df_baselines_agnews,
                       10, "lora", "tables_training/time_full/time_full_agnews_10_lora.tex")
create_time_table_full(time_df_training_agnews, time_df_baselines_agnews,
                       10, "prefix", "tables_training/time_full/time_full_agnews_10_prefix.tex")
create_time_table_full(time_df_training_sensation, time_df_baselines_sensation,
                       10, "adapter", "tables_training/time_full/time_full_sensation_10_adapter.tex")
create_time_table_full(time_df_training_sensation, time_df_baselines_sensation,
                       10, "lora", "tables_training/time_full/time_full_sensation_10_lora.tex")
create_time_table_full(time_df_training_sensation, time_df_baselines_sensation,
                       10, "prefix", "tables_training/time_full/time_full_sensation_10_prefix.tex")
create_time_table_full(time_df_training_trec, time_df_baselines_trec, 
                       10, "adapter", "tables_training/time_full/time_full_trec_10_adapter.tex")
create_time_table_full(time_df_training_trec, time_df_baselines_trec,
                       10, "lora", "tables_training/time_full/time_full_trec_10_lora.tex")
create_time_table_full(time_df_training_trec, time_df_baselines_trec,
                       10, "prefix", "tables_training/time_full/time_full_trec_10_prefix.tex")

create_speedup_table_short(time_df_training_agnews, time_df_baselines_agnews, 
                           10, "tables_training/speedup_short/speedup_short_agnews_10.tex")
create_speedup_table_short(time_df_training_sensation, time_df_baselines_sensation,
                           10, "tables_training/speedup_short/speedup_short_sensation_10.tex")
create_speedup_table_short(time_df_training_trec, time_df_baselines_trec,
                           10, "tables_training/speedup_short/speedup_short_trec_10.tex")
